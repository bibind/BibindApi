...
class SaasPayoutRun(models.Model):
    ...
    name = fields.Char(required=True, default=lambda self: fields.Date.today())
    ...
    @api.model
    def cron_prepare_monthly(self):
        """Create draft payout run monthly."""
        self.create({})
...
class SaasPayoutItem(models.Model):
    ...
    fx_rate_used = fields.Float(default=1.0)
    net_after_withholding = fields.Monetary(currency_field='currency_id')
    bank_ref = fields.Char()
    state = fields.Selection([('draft', 'Draft'), ('exported', 'Exported'), ('paid', 'Paid')], default='draft')
    ledger_ids = fields.One2many('saas.commission.ledger', 'payout_item_id')
